...
void loop() {
    ...
    lastLoopTime = millis();
    // Ensure circulation fan is always ON unless override is active
    if (!overrideActive) {
        digitalWrite(RELAY_CIRCULATION_FAN, LOW);  // Keep circulation fan ON
    }
...
void runSchedule() {
    ...
    // Circulation fan is now enforced ON in loop(), so we do not set it here
